...
import { TMMSimulator, Layer, SimulationResult, AbsorptionProfilePoint, Polarization, MATERIALS, ColorUtils, SpectralPoint } from './lib/tmm';
...
const PRESETS = {
  ...
  'Solar Cell (Simple)': [
    ...
    { id: 'p3-3', name: 'Perovskite', thickness: 300, n: 2.5, k: 0.05, isThick: false },
    { id: 'p3-4', name: 'Gold', thickness: 100, n: 0.2, k: 3.5, isThick: false },
...
export default function App() {
  ...
  const [wavelengthRange, setWavelengthRange] = useState({ min: 300, max: 1200, step: 2 });
  ...
  const [profileMode, setProfileMode] = useState<'absorption' | 'field'>('absorption');
  const [savedDesigns, setSavedDesigns] = useState<string[]>([]);
  const [editingSpectralLayerId, setEditingSpectralLayerId] = useState<string | null>(null);
  ...
  const saveMaterial = (layer: Layer) => {
    ...
    if (existingIndex >= 0) {
      ...
      const newMat = { ...layer, id: Math.random().toString(36).substr(2, 9), name };
      all.push(newMat);
    }
  ...
  const [expandedElectrical, setExpandedElectrical] = useState<Record<string, boolean>>({});
  ...
  const runSimulation = () => {
    ...
    setTimeout(() => {
      ...
      for (let w = wavelengthRange.min; w <= wavelengthRange.max; w += wavelengthRange.step) {
        newResults.push(TMMSimulator.calculate(w, angle, polarization, simulationLayers, nStart, nEnd));
      }
      setResults(newResults);

      const wavelengths = profileWavelengths.split(',').map(s => parseFloat(s.trim())).filter(n => !isNaN(n));
      if (wavelengths.length === 0) wavelengths.push(550);

      let combinedProfile: AbsorptionProfilePoint[] = [];

      wavelengths.forEach((w, idx) => {
        const p = TMMSimulator.calculateAbsorptionProfile(w, angle, polarization, simulationLayers, nStart, nEnd);
        if (idx === 0) {
  ...
  const handleSpectralUpload = (layerId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      if (data.length > 0) {
        ...
        alert('Could not parse spectral data. Format should be: wavelength n k (space or comma separated)');
      }
    };
    reader.readAsText(file);
  };

  const updateSpectralPoint = (layerId: string, pointIndex: number, updates: Partial<SpectralPoint>) => {
    setLayers(layers.map(l => {
      if (l.id === layerId && l.spectralData) {
        const newData = [...l.spectralData];
        newData[pointIndex] = { ...newData[pointIndex], ...updates };
        return { ...l, spectralData: newData.sort((a, b) => a.wavelength - b.wavelength) };
      }
      return l;
    }));
  };
  ...
  const removeSpectralPoint = (layerId: string, pointIndex: number) => {
    setLayers(layers.map(l => {
      if (l.id === layerId && l.spectralData) {
        ...
        return { ...l, spectralData: newData.length > 0 ? newData : undefined };
      }
      return l;
    }));
  };
    ...
    <div className="min-h-screen bg-[#E4E3E0] text-[#141414] font-sans selection:bg-[#141414] selection:text-[#E4E3E0]">
      ...
      <header className="border-b border-[#141414] p-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs uppercase tracking-widest opacity-50 mt-1 font-mono">Multilayer Film Simulator / TMM Engine</p>
        </div>
        <div className="flex gap-4">
          ...
          >
            ...
            <span className="font-mono text-sm uppercase tracking-wider">Run Simulation</span>
          </button>
        </div>
      </header>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 min-h-[calc(100vh-100px)] relative">
        {isSimulating && (
          <div className="absolute inset-0 bg-[#E4E3E0]/50 backdrop-blur-[2px] z-40 flex items-center justify-center">
            <div className="bg-[#141414] text-[#E4E3E0] px-6 py-4 flex items-center gap-3 shadow-2xl border border-[#E4E3E0]/20 animate-in fade-in zoom-in duration-200">
              ...
              <span className="font-mono text-xs uppercase tracking-widest">Computing Simulation...</span>
            </div>
          </div>
        )}
        {/* Sidebar: Layer Editor */}
        <div className="lg:col-span-4 border-r border-[#141414] p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => setLayers([layers[0], layers[layers.length - 1]])}
                  disabled={isSimulating}
              ...
              <div className="flex flex-col-reverse w-full h-32 border border-[#141414] overflow-hidden">
                {layers.map((l, i) => (
                  <div 
                    ...
                    style={{ 
                      height: l.thickness === 0 ? '15%' : `${Math.max(5, (l.thickness / layers.reduce((a, b) => a + b.thickness, 0)) * 70)}%`,
                      backgroundColor: i === 0 || i === layers.length - 1 ? '#eee' : `rgba(20, 20, 20, ${0.1 + (i % 5) * 0.1})`
                    }}
                  >
                    <span className="font-mono text-[8px] uppercase truncate px-1">{l.name}</span>
                  </div>
                ))}
              </div>
              <p className="font-mono text-[8px] uppercase opacity-50 mt-1 text-center">Structure Visualization</p>
            </div>
            ...
            {layers.map((layer, index) => (
              <div 
                ...
                className={cn(
                  ...
                  (index === 0 || index === layers.length - 1) ? "bg-[#D4D3D0]" : "bg-white"
                )}
              >
                <div className="flex justify-between items-center mb-3">
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        if (MATERIALS[val]) {
                          ...
                          updateLayer(layer.id, { n: mat.n, k: mat.k, name: val, spectralData: undefined });
                        } else {
                          const savedMat = savedMaterials.find(m => m.name === val);
                          if (savedMat) {
                    ...
                    >
                      ...
                      {savedMaterials.length > 0 && (
                        <optgroup label="Saved Materials">
                          {savedMaterials.map(m => <option key={m.id} value={m.name}>{m.name}</option>)}
                        </optgroup>
                      )}
                      {!Object.keys(MATERIALS).includes(layer.name) && !savedMaterials.find(m => m.name === layer.name) && (
                  ...
                  <div className="flex gap-1">
                    {savedMaterials.find(m => m.name === layer.name) && (
                      <button 
                        onClick={() => {
                          const mat = savedMaterials.find(m => m.name === layer.name);
                          if (mat && window.confirm(`Delete material "${mat.name}"?`)) {
                ...
                <div className="grid grid-cols-4 gap-2">
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Thickness</label>
                    <div className="relative">
                      <input 
                        type="number"
                        disabled={index === 0 || index === layers.length - 1 || isSimulating}
                      ...
                      />
                      <span className="absolute right-1 top-1 font-mono text-[8px] opacity-30">nm</span>
                    </div>
                  </div>
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">n</label>
                    <input 
                      type="number"
                      step="0.01"
                      disabled={isSimulating}
                      value={layer.n}
                      onChange={(e) => updateLayer(layer.id, { n: Number(e.target.value) })}
                      className="w-full font-mono text-xs border border-[#141414] p-1 bg-transparent disabled:opacity-50"
                    />
                  </div>
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">k</label>
                    <input 
                      type="number"
                      step="0.001"
                      disabled={isSimulating}
                      value={layer.k}
                      onChange={(e) => updateLayer(layer.id, { k: Number(e.target.value) })}
                      className="w-full font-mono text-xs border border-[#141414] p-1 bg-transparent disabled:opacity-50"
                    />
                  </div>
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Coherence</label>
                    <button 
                      disabled={index === 0 || index === layers.length - 1 || isSimulating}
                      onClick={() => updateLayer(layer.id, { isThick: !layer.isThick })}
                      className={cn(
                        "w-full font-mono text-[9px] border border-[#141414] p-1 uppercase disabled:opacity-30",
                        layer.isThick ? "bg-[#141414] text-white" : "bg-transparent"
                      )}
                    >
                  ...
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">BW (nm)</label>
                    <input 
                      type="number"
                      step="1"
                      disabled={isSimulating}
                      value={layer.bandwidth || 0}
                      onChange={(e) => updateLayer(layer.id, { bandwidth: Number(e.target.value) })}
                      className="w-full font-mono text-xs border border-[#141414] p-1 bg-transparent disabled:opacity-50"
                    />
                  </div>
                  <div className="col-span-1">
                    <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Coherence</label>
                    <button 
                      disabled={index === 0 || index === layers.length - 1 || layer.isThick || isSimulating}
                      onClick={() => updateLayer(layer.id, { forceIncoherent: !layer.forceIncoherent })}
                      className={cn(
                        "w-full font-mono text-[9px] border border-[#141414] p-1 uppercase disabled:opacity-30",
                        layer.forceIncoherent ? "bg-amber-100 text-amber-800 border-amber-600" : "bg-transparent"
                      )}
                    >
                  ...
                  <div className="mt-3 pt-3 border-t border-[#141414]/10 flex justify-between items-center">
                    <button 
                      ...
                      className={cn(
                        ...
                        expandedElectrical[layer.id] ? "bg-[#141414] text-[#E4E3E0]" : "hover:bg-gray-50"
                      )}
                    >
                      <Zap className="w-3 h-3" />
                      <span className="font-mono text-[9px] uppercase">Electrical Params</span>
                    </button>
                    ...
                    <div className="relative">
                      ...
                      <button 
                        ...
                        className={cn(
                          "flex items-center gap-1.5 px-2 py-1 border border-[#141414] transition-all disabled:opacity-50",
                          layer.spectralData ? "bg-emerald-50 border-emerald-600 text-emerald-700" : "hover:bg-gray-50"
                        )}
                      >
                        <BarChart3 className="w-3 h-3" />
                        <span className="font-mono text-[9px] uppercase">
                          {layer.spectralData ? `Data (${layer.spectralData.length})` : 'Spectral Data'}
                        </span>
                      </button>
                      {layer.spectralData && (
                        <div className="flex gap-1 absolute -right-1 -top-1">
                          ...
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              updateLayer(layer.id, { spectralData: undefined });
                            }}
                            disabled={isSimulating}
                ...
                {expandedElectrical[layer.id] && (
                  <div className="mt-3 grid grid-cols-2 gap-2 p-3 bg-gray-50 border border-[#141414]/10 animate-in fade-in slide-in-from-top-1 duration-200">
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">Bandgap (eV)</label>
                      <input 
                        type="number" step="0.1"
                        disabled={isSimulating}
                        value={layer.bandgap || 0}
                        onChange={(e) => updateLayer(layer.id, { bandgap: Number(e.target.value) })}
                        className="w-full font-mono text-[10px] border border-[#141414]/20 p-1 bg-white disabled:opacity-50"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">Affinity (eV)</label>
                      <input 
                        type="number" step="0.1"
                        disabled={isSimulating}
                        value={layer.affinity || 0}
                        onChange={(e) => updateLayer(layer.id, { affinity: Number(e.target.value) })}
                        className="w-full font-mono text-[10px] border border-[#141414]/20 p-1 bg-white disabled:opacity-50"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">Doping</label>
                      <div className="flex border border-[#141414]/20">
                        {['n', 'i', 'p'].map(t => (
                          <button
                            key={t}
                            disabled={isSimulating}
                            onClick={() => updateLayer(layer.id, { dopingType: t as any })}
                            className={cn(
                              "flex-1 font-mono text-[8px] uppercase p-1 disabled:opacity-50",
                              (layer.dopingType || 'i') === t ? "bg-[#141414] text-white" : "bg-white hover:bg-gray-100"
                            )}
                          >
                            {t}
                          </button>
                    ...
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">Conc. (cm⁻³)</label>
                      <input 
                        type="number"
                        disabled={isSimulating}
                        value={layer.dopingConc || 0}
                        onChange={(e) => updateLayer(layer.id, { dopingConc: Number(e.target.value) })}
                        className="w-full font-mono text-[10px] border border-[#141414]/20 p-1 bg-white disabled:opacity-50"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">μₙ (cm²/Vs)</label>
                      <input 
                        type="number"
                        disabled={isSimulating}
                        value={layer.mobilityN || 0}
                        onChange={(e) => updateLayer(layer.id, { mobilityN: Number(e.target.value) })}
                        className="w-full font-mono text-[10px] border border-[#141414]/20 p-1 bg-white disabled:opacity-50"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="block font-mono text-[8px] uppercase opacity-50">μₚ (cm²/Vs)</label>
                      <input 
                        type="number"
                        disabled={isSimulating}
                        value={layer.mobilityP || 0}
                        onChange={(e) => updateLayer(layer.id, { mobilityP: Number(e.target.value) })}
                        className="w-full font-mono text-[10px] border border-[#141414]/20 p-1 bg-white disabled:opacity-50"
                      />
                    </div>
                  </div>
          ...
          <div className="mt-8 pt-8 border-t border-[#141414]">
            ...
            <div className="space-y-2">
              {savedDesigns.length === 0 && <p className="font-mono text-[10px] opacity-30 italic">No saved designs yet.</p>}
          ...
          <div className="mt-8 pt-8 border-t border-[#141414]">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Wavelength Min (nm)</label>
                  <input 
                    type="number"
                    value={wavelengthRange.min}
                    onChange={(e) => setWavelengthRange({ ...wavelengthRange, min: Number(e.target.value) })}
                    disabled={isSimulating}
                    className="w-full font-mono text-xs border border-[#141414] p-2 bg-transparent disabled:opacity-50"
                  />
                </div>
                <div>
                  <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Wavelength Max (nm)</label>
                  <input 
                    type="number"
                    value={wavelengthRange.max}
                    onChange={(e) => setWavelengthRange({ ...wavelengthRange, max: Number(e.target.value) })}
                    disabled={isSimulating}
                    className="w-full font-mono text-xs border border-[#141414] p-2 bg-transparent disabled:opacity-50"
                  />
                </div>
              ...
              <div>
                <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Incident Angle (deg)</label>
                <input 
                  type="range"
                  min="0"
                  max="89"
              ...
              <div>
                <label className="block font-mono text-[9px] uppercase opacity-50 mb-1">Polarization</label>
                <div className="flex border border-[#141414]">
                  <button 
                    ...
                    className={cn(
                      ...
                      polarization === 's' ? "bg-[#141414] text-[#E4E3E0]" : "hover:bg-gray-100"
                    )}
                  >
                    S-Pol
                  </button>
                  <button 
                    ...
                    className={cn(
                      "flex-1 py-2 font-mono text-xs uppercase tracking-widest transition-all disabled:opacity-50",
                      polarization === 'p' ? "bg-[#141414] text-[#E4E3E0]" : "hover:bg-gray-100"
                    )}
                  >
                    P-Pol
                  </button>
        ...
        <div className="lg:col-span-8 p-6 space-y-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
            <div className="xl:col-span-3 bg-white border border-[#141414] p-6">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                  <BarChart3 className="w-5 h-5" />
                  <h2 className="font-serif italic text-xl">Optical Response</h2>
                </div>
                <div className="flex gap-4 font-mono text-[10px] uppercase opacity-50">
                  <div className="flex items-center gap-1"><div className="w-2 h-2 bg-[#141414]"></div> Reflection</div>
                  <div className="flex items-center gap-1"><div className="w-2 h-2 bg-[#F27D26]"></div> Transmission</div>
                  <div className="flex items-center gap-1"><div className="w-2 h-2 bg-[#00FF00]"></div> Absorption</div>
                </div>
              </div>
              ...
              <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={results} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <XAxis 
                      dataKey="wavelength" 
                      label={{ value: 'Wavelength (nm)', position: 'insideBottom', offset: -5, fontSize: 10, fontFamily: 'monospace' }}
                    ...
                    />
                    <Tooltip 
                      contentStyle={{ fontFamily: 'monospace', fontSize: '10px', borderRadius: 0, border: '1px solid #141414' }}
                    />
                    <Line type="monotone" dataKey="R" stroke="#141414" strokeWidth={2} dot={false} name="Reflection" />
                    <Line type="monotone" dataKey="T" stroke="#F27D26" strokeWidth={2} dot={false} name="Transmission" />
                    <Line type="monotone" dataKey="A" stroke="#00FF00" strokeWidth={2} dot={false} name="Absorption" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#141414] p-6 flex flex-col items-center justify-center text-center">
              <h3 className="font-mono text-[10px] uppercase opacity-50 mb-4 tracking-widest">Reflected Color</h3>
              <div 
              ...
              />
              <p className="font-mono text-[10px] opacity-50 uppercase">{reflectedColor}</p>
              <p className="font-serif italic text-xs mt-2 opacity-80">Predicted appearance under white light</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#141414] p-6">
                ...
                <div className="flex border border-[#141414] mr-4">
                  <button 
                    ...
                    className={cn(
                      ...
                      profileMode === 'absorption' ? "bg-[#141414] text-[#E4E3E0]" : "hover:bg-gray-100"
                    )}
                  >
                    Absorption
                  </button>
                  <button 
                    ...
                    className={cn(
                      "px-3 py-1 font-mono text-[9px] uppercase tracking-widest transition-all disabled:opacity-50",
                      profileMode === 'field' ? "bg-[#141414] text-[#E4E3E0]" : "hover:bg-gray-100"
                    )}
                  >
                    |E|² Field
                  </button>
                </div>
                <label className="font-mono text-[9px] uppercase opacity-50">At Wavelength(s):</label>
                <input 
                  type="text"
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={profile} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <defs>
                  ...
                  <XAxis 
                    dataKey="z" 
                    label={{ value: 'Depth (nm)', position: 'insideBottom', offset: -5, fontSize: 10, fontFamily: 'monospace' }}
                    tick={{ fontSize: 10, fontFamily: 'monospace' }}
                  />
                  <YAxis 
                    tick={{ fontSize: 10, fontFamily: 'monospace' }}
                  ...
                  <Tooltip 
                    contentStyle={{ fontFamily: 'monospace', fontSize: '10px', borderRadius: 0, border: '1px solid #141414' }}
                    formatter={(value: number) => [value.toFixed(4), profileMode === 'absorption' ? 'Abs. Rate' : '|E|²']}
                  />
                  <Area 
                    type="monotone" 
                    dataKey={profileMode === 'absorption' ? "absorption" : "fieldIntensity"} 
                    stroke="#00FF00" 
                    fillOpacity={1} 
                    fill="url(#colorAbs)" 
                    name={profileMode === 'absorption' ? "Absorption Rate" : "Electric Field Intensity"} 
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <p className="font-mono text-[10px] opacity-50 mt-4 italic">
              * The absorption profile shows where light is being absorbed within the structure at {selectedWavelength}nm.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border border-[#141414] p-6 bg-[#D4D3D0]">
              ...
              <p className="text-sm leading-relaxed opacity-80">
                This simulator uses the <strong>Transfer Matrix Method (TMM)</strong> to solve Maxwell's equations in planar multilayer structures. 
                It accounts for multiple internal reflections and interference effects, which are critical for thin films where layer thickness is comparable to the wavelength of light.
              </p>
            </div>
            <div className="border border-[#141414] p-6 bg-[#D4D3D0]">
              <h3 className="font-serif italic text-lg mb-2 flex items-center gap-2">
              ...
              <p className="text-sm leading-relaxed opacity-80 mb-4">
                Download the simulation results as a CSV file for further analysis in external tools.
              </p>
              <button 
                onClick={() => {
                  const csvContent = "data:text/csv;charset=utf-8," 
                    + "Wavelength,Reflection,Transmission,Absorption\n"
                    + results.map(r => `${r.wavelength},${r.R},${r.T},${r.A}`).join("\n");
            ...
            <div className="p-4 border-b border-[#141414] flex justify-between items-center bg-[#141414] text-[#E4E3E0]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-serif italic text-xl">
                  Spectral Data: {layers.find(l => l.id === editingSpectralLayerId)?.name}
                </h2>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-1/2 border-r border-[#141414] flex flex-col">
                <div className="p-4 border-b border-[#141414] flex justify-between items-center bg-white/50">
                  <span className="font-mono text-[10px] uppercase opacity-50">Data Points</span>
                  <div className="flex gap-2">
                    <input 
                      type="file" 
                    ...
                    />
                    <button 
                      onClick={() => document.getElementById(`spectral-upload-modal`)?.click()}
                  ...
                  <table className="w-full font-mono text-[10px]">
                    <thead className="sticky top-0 bg-[#E4E3E0] z-10">
                      <tr className="border-b border-[#141414]">
                        <th className="text-left p-2 uppercase opacity-50">λ (nm)</th>
                        <th className="text-left p-2 uppercase opacity-50">n</th>
                        <th className="text-left p-2 uppercase opacity-50">k</th>
                        <th className="w-8"></th>
                      </tr>
                    </thead>
                    <tbody>
                      {layers.find(l => l.id === editingSpectralLayerId)?.spectralData?.map((point, idx) => (
                        <tr key={idx} className="border-b border-[#141414]/10 hover:bg-white/30">
                          <td className="p-1">
                            <input 
                              type="number"
                              value={point.wavelength}
                              onChange={(e) => updateSpectralPoint(editingSpectralLayerId, idx, { wavelength: Number(e.target.value) })}
                          ...
                          <td className="p-1">
                            <input 
                              ...
                              step="0.01"
                              value={point.n}
                              onChange={(e) => updateSpectralPoint(editingSpectralLayerId, idx, { n: Number(e.target.value) })}
                              className="w-full bg-transparent p-1 focus:bg-white outline-none"
                            />
                          </td>
                          <td className="p-1">
                            <input 
                              ...
                              step="0.001"
                              value={point.k}
                              onChange={(e) => updateSpectralPoint(editingSpectralLayerId, idx, { k: Number(e.target.value) })}
                              className="w-full bg-transparent p-1 focus:bg-white outline-none"
                            />
                          </td>
                          <td className="p-1">
                            <button 
                              onClick={() => removeSpectralPoint(editingSpectralLayerId, idx)}
              ...
              <div className="w-full md:w-1/2 flex flex-col bg-white">
                <div className="p-4 border-b border-[#141414] bg-gray-50">
                  <span className="font-mono text-[10px] uppercase opacity-50">Visualization (n & k)</span>
                </div>
                <div className="flex-1 p-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={layers.find(l => l.id === editingSpectralLayerId)?.spectralData || []}>
                      ...
                      <YAxis 
                        yId="left"
                        label={{ value: 'n', angle: -90, position: 'insideLeft', fontSize: 10, fontFamily: 'monospace' }}
                        tick={{ fontSize: 10, fontFamily: 'monospace' }}
                      />
                      <YAxis 
                        yId="right"
                        orientation="right"
                        label={{ value: 'k', angle: 90, position: 'insideRight', fontSize: 10, fontFamily: 'monospace' }}
                        tick={{ fontSize: 10, fontFamily: 'monospace' }}
                      />
                      <Tooltip 
                        contentStyle={{ fontSize: '10px', fontFamily: 'monospace', border: '1px solid #141414' }}
                      />
